...
for i in indels:
    ...
    entry = '\t'.join([chrom_name, str(before_indel), '.', reference_state, alternate_state, '.', '.', 'INDEL'])
    handle.write(entry + '\n')
handle.close()
...
for position in snp_positions:
    ...
    if reference_allele == '-' or alternate_allele == '-':
        continue
    entry = '\t'.join([chrom_name, str(position), '.', reference_allele, alternate_allele, '.', '.', '.'])
    handle.write(entry + '\n')
handle.close()
