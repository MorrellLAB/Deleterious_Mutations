...
with open(sys.argv[1], 'r') as f:
    for line in f:
        if line.startswith('#'):
            ...
            snp_id = tmp[2]
            alt = tmp[4]
            vcf_snps.append((snp_id, chrom, start, alt))
...
#   Iterate through the list of SNPs from the VCF
...
for each in vcf_snps:
    ...
    if coding:
        ...
        if snp_ref_position not in cds_reference_positions:
            ...
            codon_number = str(int(math.ceil(snp_cds_position / 3)))
            ...
            #   If there are Ns in the codon, we skip it!
            if 'N' in codons[0] or 'N' in codons[1]:
                silent = 'NA'
                translations = ['-', '-']
                position = 'Ns in Codons!'
            else:
    ...
    #   If we are not coding
        ...
        position = 'Not_in_CDS'
    #   Print it all out
    information = [each[0], position, translations[0], translations[1], codons[0], codons[1], codon_number, silent]
